...
metadata {
	...
	tiles {
        ...
        standardTile("mute", "device.mute", width: 1, height: 1, canChangeIcon: false, canChangeBackground: false) {
        ...
        standardTile("input", "device.input", width: 1, height: 1, canChangeIcon: false, canChangeBackground: false) {
...
// sleep up to 9 seconds before issuing the next command
    ...
    if(cur > 0 && cur > c.time.time && cur - c.time.time < 3000) { 
    	waitTime = cur - c.time.time
		cmds << "delay $waitTime"
    }
    c = new GregorianCalendar()
    sendEvent(name: "blocked", value: c.time.time + 3000 + waitTime)
