...
metadata {
	...
	tiles {
		standardTile("switch", "device.switch", width: 2, height: 2, canChangeIcon: true) {
   		...
   		standardTile("input", "device.switch", inactiveLabel: false, decoration: "flat") {
			state "default", label:'', action:"roku.roku", icon:"st.secondary.refresh"
		}
...
def roku() {
	log.debug "Executing 'roku'"
	// TODO: handle 'refresh' command
    new physicalgraph.device.HubAction("${settings.username}\r${settings.password}\rIAVD1   \r", physicalgraph.device.Protocol.LAN)
}